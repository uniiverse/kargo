 ...
 * the time range is bounded to the run's lifetime. Returns undefined when the
 * server has no Grafana URL configured, or when the run's job context can't
 * be resolved.
 */
export const buildDashboardUrl = (
  ...
  return `${baseUrl}/d/${verificationDashboardUid()}?${params.toString()}`;
};
 ...
 * time range. Returns undefined when the server has no Grafana URL
 * configured, or when the run's job context can't be resolved.
 *
 * The `?left=<json>` schema is a Grafana-versioned contract. It is validated
 * against Grafana v12; a major Grafana upgrade can change the Explore state
 * encoding and silently break this link. The dashboard link is the primary
 * affordance precisely because it hides this behind a stable /d/<uid> URL.
 * If the target Grafana moves past v12, re-verify this encoding.
 */
export const buildExploreUrl = (
  ...
  const lokiUid = lokiDatasourceUid();

  // Unescaped '.' in the pod regex is intentional: matches the deployed
  // inline query used by the verification-logs dashboard so the deep-link
  // and Kargo's inline log view use identical LogQL. Job names are
  // DNS-label-safe, so false matches can't occur in practice.
  const expr =
    `{cluster="${verificationCluster()}", namespace="${ctx.jobNamespace}"} ` +
    `| pod=~"${ctx.jobName}.*"`;
  ...
  return `${baseUrl}/explore?left=${encodeURIComponent(JSON.stringify(left))}`;
};
