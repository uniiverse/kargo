...
| `kustomize.enableAlphaPlugins` | `boolean` | N | Enables the use of Kustomize alpha plugins such as container-based KRM function plugins. This is equivalent to the `--enable-alpha-plugins` flag in the kustomize CLI. Defaults to `false`. Exec-based KRM functions are not permitted. This option only takes effect when the controller is running with the `KUSTOMIZE_ENABLE_ALPHA_PLUGINS=true` environment variable. |
